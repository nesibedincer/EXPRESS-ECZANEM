    ...
    <div class="topnav" id="myTopnav">
        ...
        <div class="dropdown">
            ...
            <div class="dropdown-content">
                <a href="eczaci_profil.php">Profil</a>
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['orderId']) && isset($_POST['status'])) {
    ...
    $updateQuery = $db->prepare("UPDATE siparisler SET sip_durum = :status WHERE sip_no = :orderId");
    ...
    if ($updateQuery->execute()) {
        ...
        echo "Sipariş durumu güncellenirken bir hata oluştu.";
    }
}

// Sipariş durumu "beklemede" olanları çek
$query = $db->prepare("SELECT * FROM siparisler WHERE sip_durum = 'beklemede'");
$query->execute();
$bekleyenSiparisler = $query->fetchAll(PDO::FETCH_ASSOC);

// Bekleyen siparişler var mı kontrol et
if ($bekleyenSiparisler) {
    // Bekleyen siparişlerin bilgilerini ekrana yazdır
    foreach ($bekleyenSiparisler as $siparis) {
        ...
        echo "<button type='submit' name='status' value='onaylandi'>Onayla</button>";
        echo "<button type='submit' name='status' value='reddedildi'>Reddet</button>";
        echo "</form>";
        echo "</div>";
        echo "</div>";
        echo "<hr>";
    ...
    echo "<p>Bekleyen sipariş bulunmamaktadır.</p>";
}
?>
