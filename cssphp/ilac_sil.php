...
// Oturum açılmış ve kullanıcı türü "eczaci" olarak belirlenmişse
if (isset($_SESSION["tur"]) && $_SESSION["tur"] == 'eczaci') {
    // İlaç silme işlemleri
    if (isset($_POST['sil'])) {
        $barkod = $_POST['barkod'];
        ...
        // İlaç silme sorgusu
        $sil = $db->prepare("DELETE FROM ilaclar WHERE ilac_barkod = :barkod");
        $sil->bindParam(':barkod', $barkod);
        $sil->execute();

        if ($sil) {
            echo "İlaç başarıyla silindi.";
        } else {
            echo "İlaç silme hatası!";
        }
    }
?>
