...
if ($_SESSION["tur"] == 'eczaci') {
    ...
    if (isset($_POST['guncelle'])) {
        ...
        if ($_FILES['foto']['size'] > 0) {
            ...
            move_uploaded_file($foto_tmp, "upload/$foto");

            // Veritabanı bağlantısı
            $db = new PDO("mysql:host=localhost;dbname=eczanem", 'root', ''); 

            // İlaç güncelleme sorgusu (fotoğraf güncellenecek)
            $guncelle = $db->prepare("UPDATE ilaclar SET ilac_adi = :ilac_adi, ilac_fiyat = :fiyat, ilac_miktari = :miktar, foto = :foto WHERE ilac_barkod = :barkod");
            $guncelle->bindParam(':barkod', $barkod);
            $guncelle->bindParam(':ilac_adi', $ilac_adi);
            $guncelle->bindParam(':fiyat', $fiyat);
            $guncelle->bindParam(':miktar', $miktar);
            $guncelle->bindParam(':foto', $foto);
            $guncelle->execute();
        } else {
            ...
            // İlaç güncelleme sorgusu (fotoğraf güncellenmeyecek)
            $guncelle = $db->prepare("UPDATE ilaclar SET ilac_adi = :ilac_adi, ilac_fiyat = :fiyat, ilac_miktari = :miktar WHERE ilac_barkod = :barkod");
            $guncelle->bindParam(':barkod', $barkod);
            $guncelle->bindParam(':ilac_adi', $ilac_adi);
            $guncelle->bindParam(':fiyat', $fiyat);
            $guncelle->bindParam(':miktar', $miktar);
        ...
        if ($guncelle) {
            echo "İlaç başarıyla güncellendi.";
        } else {
            echo "İlaç güncelleme hatası!";
        }
    }
?>
...
cover; background-position: center; background-repeat: no-repeat;">
    ...
    <section style="color:white" id='update-section'>
        <div class='update-container'>
            <form id='update-form' action='' method='post' enctype='multipart/form-data'>  
                <label for='barkod'>İlaç Barkod No:</label>
                <input type='text' name='barkod' class='girdi' required>
                ...
                <label for='fiyat'>Yeni İlaç Fiyatı:</label>
                <input type='text' name='fiyat' class='girdi' required>

                <label for='miktar'>Yeni İlaç Miktarı:</label>
                <input type='text' name='miktar' class='girdi' required>

                <label for='foto'>Fotoğraf Seçiniz:</label>
                <input type='file' name='foto'>

                <button type='submit' name='guncelle' class='btn'>İlaç Güncelle</button>
            </form>
        </div>
    </section>
 <footer>
